...
begin
  ...
  begin
    ...
    begin
      ...
      if ((TExprWord(Items[I]).Name = '-') or (TExprWord(Items[I]).Name = '+')) and ((I = 0) or (TExprWord(Items[I - 1]).VarType = vtComma) or (TExprWord(Items[I - 1]).VarType = vtLeftBracket) or (TExprWord(Items[I - 1]).IsOper and (TExprWord(Items[I - 1]).NFunctionArg = 2))) then
      begin
        ...
        if TExprWord(Items[I]).Name = '-' then
          ...
          K := 1;
        L := 1;
        while (I + L < Count) and ((TExprWord(Items[I + L]).Name = '-') or (TExprWord(Items[I + L]).Name = '+')) and ((I + L = 0) or (TExprWord(Items[I + L - 1]).VarType = vtComma) or (TExprWord(Items[I + L - 1]).VarType = vtLeftBracket) or (TExprWord(Items[I + L - 1]).IsOper and (TExprWord(Items[I + L - 1]).NFunctionArg = 2))) do
        begin
      ...
      begin
        ...
        K := -1;
        L := 1;
        while (I + L < Count) and (TExprWord(Items[I + L]).Name = 'not') and ((I + L = 0) or (TExprWord(Items[I + L - 1]).VarType = vtLeftBracket) or TExprWord(Items[I + L - 1]).IsOper) do
        begin
          K := -K;
          Inc(L);
        end;
      ...
      {-----CHECK ON INTPOWER------}
      if (TExprWord(Items[I]).Name = '^') and ((I < Count - 1) and (TExprWord(Items[I + 1]).ClassType = TDoubleConstant) and (Pos(DecimSeparator, TExprWord(Items[I + 1]).Name) = 0)) then
        if WordsList.Search(pchar('^@'), J) then
    ...
    begin
      ...
      begin
        ...
        else if (I + 2 < Count) and (TExprWord(Items[I + 2]).VarType = vtString) then
        begin
